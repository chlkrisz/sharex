...
export async function uploadFile(fileName: string) {
    ...
    const response = await axios.put(`${settings.base_url}/${settings.storage_zone_name}/${settings.upload_path}${newName}`, fileStream, {
        headers: {
...
export async function deleteFile(filePath: string) {
    const response = await axios.delete(`${settings.base_url}/${settings.storage_zone_name}/${filePath}`, {
        headers: {
            'AccessKey': `${process.env.BUNNY_ACCESS_TOKEN}`,
            'Accept': 'application/json'
        }
